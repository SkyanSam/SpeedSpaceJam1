...
try
{
    ...
    while (!WindowShouldClose() || !Globals.quitGame)    // Detect window close button or ESC key
    {
        ...
        if (IsKeyDown(KeyboardKey.KEY_P) && !pKeyDownPrevious)
        {
            ...
            pKeyDownPrevious = true;
        }
        
        Console.WriteLine($"is paused {Globals.isPaused}");
        if (!Globals.isPaused)
        {
            AudioManager.Update();
            ...
            if (UIManager.state == UIManager.State.DrawScore)
            {
                ...
                var endPos = MazeBuilder.Get("G2II");
                Raylib.DrawText("END", (int)endPos.X, (int)endPos.Y, 50, Color.GREEN);
                if (Globals.player.position.X > MazeBuilder.Get("G2III").X && UIManager.state == UIManager.State.DrawScore)
                {
                    Score.GetLeaderboard();
                    UIManager.state = UIManager.State.DrawFinalScore;
